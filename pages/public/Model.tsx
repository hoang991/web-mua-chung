...
import { ArrowRight, Package, UserCheck, Users, Truck, Calendar } from 'lucide-react';
...
const Model = () => {
  ...
  // Helper to get icon by index - since we can't easily store icon components in JSON
  const getIcon = (idx: number) => {
      const icons = [Calendar, Package, Truck, UserCheck];
      return icons[idx % icons.length];
  };

  return (
    <>
      <div className="bg-emerald-900 text-emerald-50 py-16">
      ...
      <Section>
        <Container>
          ...
          {diagramSection && diagramSection.isVisible && (
            <FadeIn>
              <div className="max-w-4xl mx-auto bg-white p-8 md:p-12 rounded-2xl shadow-sm border border-stone-200 mb-20">
                <h2 className="text-2xl font-bold text-center mb-12">{diagramSection.title}</h2>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-8 relative">
                  ...
                  {/* Nodes */}
                  <div className="flex flex-col items-center bg-white p-4 z-10">
                    <div className="w-16 h-16 bg-blue-100 text-blue-700 rounded-full flex items-center justify-center mb-4 border-4 border-white shadow-sm">
                  ...
                  <div className="hidden md:flex text-stone-400">
                      <ArrowRight className="w-6 h-6 animate-pulse" />
                  </div>

                  <div className="flex flex-col items-center bg-white p-4 z-10">
                    <div className="w-16 h-16 bg-emerald-700 text-white rounded-full flex items-center justify-center mb-4 border-4 border-white shadow-lg scale-110">
                  ...
                  <div className="flex flex-col items-center bg-white p-4 z-10">
                    <div className="w-16 h-16 bg-amber-100 text-amber-700 rounded-full flex items-center justify-center mb-4 border-4 border-white shadow-sm">
                  ...
                  <div className="flex flex-col items-center bg-white p-4 z-10">
                    <div className="w-16 h-16 bg-stone-100 text-stone-700 rounded-full flex items-center justify-center mb-4 border-4 border-white shadow-sm">
            ...
            <div className="grid lg:grid-cols-2 gap-16">
              <div className="space-y-8">
                <div className="sticky top-24">
                  <h2 className="text-3xl font-bold mb-4">{timelineSection.title}</h2>
                  ...
                  <Link to="/products">
                    <Button>Xem sản phẩm kỳ này</Button>
                  </Link>
                </div>
              </div>

              <div className="relative border-l-2 border-emerald-200 pl-8 space-y-12 py-4">
                {timelineItems.map((step, idx) => (
                  <FadeIn key={idx} delay={idx * 100}>
                    <div className="relative">
                      <div className="absolute -left-[41px] top-0 bg-white border-2 border-emerald-500 w-6 h-6 rounded-full"></div>
                      <span className="text-sm font-bold text-emerald-600 tracking-wider uppercase mb-1 block">{step.label}</span>
                ...
                {timelineItems.length === 0 && (
                  <p className="text-stone-500 italic">Chưa có nội dung quy trình. Vui lòng cập nhật trong Admin.</p>
                )}
              </div>
            </div>
          )}
      ...
      <Section className="bg-stone-100">
        <Container className="max-w-4xl">
          <h2 className="text-center text-3xl font-bold mb-12">Những điều thường bị hiểu nhầm</h2>
          <div className="grid md:grid-cols-2 gap-6">
            <Card className="p-6 border-l-4 border-red-500">
              <h4 className="font-bold text-lg mb-2 flex items-center gap-2">
                <span className="text-red-500">✕</span> Hiểu nhầm: Đây là bán hàng đa cấp
              </h4>
            ...
            <Card className="p-6 border-l-4 border-red-500">
              <h4 className="font-bold text-lg mb-2 flex items-center gap-2">
                <span className="text-red-500">✕</span> Hiểu nhầm: Đầu tư tài chính
              </h4>
              <p className="text-sm text-stone-600">
